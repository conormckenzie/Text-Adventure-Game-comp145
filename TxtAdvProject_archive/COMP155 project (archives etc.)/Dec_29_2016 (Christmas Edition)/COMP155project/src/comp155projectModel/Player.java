...
public class Player {
    ...
    final String inventoryEmptyText = "You have nothing to drop. ";
    ...
    final int inventorySize = 50;
    final int ITEM_BONUS = 10;
    ...
    void pickUpItem() {
        ...
        if (!currentLocation.item.equals("")) { //redundant test?
            if (currentLocation.itemIsMovable) {
                boolean inventoryIsFull = true;
                for (String item:itemsHeld)
                {
                    if (item.equals("") && !currentLocation.item.equals("")) {
                        score += ITEM_BONUS;
                        inventoryIsFull = false;
                        item=gameLocalAlias.getHeldItemName(currentLocation.item);
                        currentLocation.item = "";
                        controllerLocalAlias.displayText("\t" + itemPickedUpText + item + ". ");         
                    }
                }
                if (inventoryIsFull) {controllerLocalAlias.displayText("\t" + inventoryFullText);}
            }
    ...
    // Something is dysfunctional about this code or a related function. 
    // Picking up the necklace and dropping it results in dropping a NULL_ITEM, and the necklace is unable to be picked up again.
    //"DROP" command has been obscured (by changing its text to something ridiculous that is not "drop") for this reason."
    void dropItem() {
        if (currentLocation.item.equals("")) {
            for(String item:itemsHeld) {System.out.println("Item = " + item);}
            if (!(itemsHeld[0].equals("") && itemsHeld[1].equals("")))
            {
                controllerLocalAlias.displayText("\t" + itemDroppedText + gameLocalAlias.getDroppedItemName(itemsHeld[0] + ". "));
            ...
            else {controllerLocalAlias.displayText("\t" + inventoryEmptyText);}
        }
